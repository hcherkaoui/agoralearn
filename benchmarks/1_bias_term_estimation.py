...
def main(
    ...
    to_return = [
        ("Empirical estimation error (unbiased)", 'tab:blue', np.array(err_est_nn_biased)),
        ("Empirical estimation error (shrinked)", 'tab:red', np.array(err_est_shrinked)),
        ("Empirical estimation error (biased)", 'tab:orange', np.array(err_est_biased)),
        ]
...
if __name__ == '__main__':
    ...
    kwargs = dict(tt=tt, X1=X1, X2=X2, theta1=theta1, theta2=theta2,
                  sigma=args.sigma, lbda=args.lbda, random_state=random_state)
...
# Plotting 1
    ...
    filename = os.path.join(figures_dir, "err_bias_term") + filename_suffix + '.pdf'
